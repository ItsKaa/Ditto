...
namespace Ditto.Bot.Services
{
    public partial class CacheHandler
    {
        ...
        public TimeSpan RefreshDelay => _refreshDelay ?? (_refreshDelay = TimeSpan.FromMinutes(5)).Value;

        private DatabaseCache _databaseCache;
        public DatabaseCache Db => _databaseCache ?? (_databaseCache = new DatabaseCache());
        public DatabaseCache Database => Db;

        public void Setup(TimeSpan refreshDelay)
        {
            _refreshDelay = refreshDelay;
        }

        private static string GetCacheName(ulong? id, [CallerMemberName] string propertyName = "") => $"{propertyName}{(id == null ? "" : "_")}{id}";
        private static string GetCacheName(IGuild guild, [CallerMemberName] string propertyName = "") => GetCacheName(guild?.Id, propertyName);
    }
    ...
    public partial class CacheHandler
    {
        public class DatabaseCache
        {
            ...
            #region Properties
            public Color EmbedColour(IGuild guild) => GetOrAddCachededItem(GetCacheName(guild),
                (uow) => uow.Configs.GetEmbedColour(guild).Value.ToColour()
                , Globals.Cache.DefaultCacheTime
            );

            public Color EmbedErrorColour(IGuild guild) => GetOrAddCachededItem(GetCacheName(guild),
                (uow) => uow.Configs.GetEmbedErrorColour(guild).Value.ToColour()
                , Globals.Cache.DefaultCacheTime
            );

            public Color EmbedRssColour(IGuild guild) => GetOrAddCachededItem(GetCacheName(guild),
                (uow) => uow.Configs.GetEmbedRssColour(guild).Value.ToColour()
                , Globals.Cache.DefaultCacheTime
            );

            public Color EmbedDiscordLinkColour(IGuild guild) => GetOrAddCachededItem(GetCacheName(guild),
